...
fb_likes_data = json.dumps(read_last_line_as_json("fb_likes_data.txt"))
with open("fb_likes_data.json", 'w') as json_file:
    json.dump(fb_likes_data, json_file)
    
fb_photos_data = read_last_line_as_json("fb_photos_data.txt")
with open("fb_photos_data.json", 'w') as json_file:
    json.dump(fb_photos_data, json_file)

fb_posts_data = read_last_line_as_json("fb_posts_data.txt")   
with open("fb_posts_data.json", 'w') as json_file:    
    json.dump(fb_posts_data, json_file)

fb_profile_data = read_last_line_as_json("fb_profile_data.txt")
with open("fb_profile_data.json", 'w') as json_file:
    json.dump(fb_profile_data, json_file)


#Construct Profile
#https://developers.facebook.com/docs/graph-api/reference/user
with open('fb_profile_data.json', 'r') as file:
    print("Constructing Profile...",flush=True)
    time.sleep(1)
    d = json.load(file)
    print("curl -X POST \"https://graph.instagram.com/v18.0/"+page_id+"/me\" -H \"Content-Type: application/json\" -d \'{\"name="+d['name']+"\", \"gender="+d['gender']+"\", \"languages="+d['languages'][0]['name']+"\", \"hometown="+d['hometown']['name']+"\", \"sports="+d['sports'][0]['name']+"\", \"location="+d['location']['name']+"\", \"birthday="+d['birthday']+"\"",flush=True)
    print("\n\n",flush=True)
...
with open('fb_posts_data.json', 'r') as file:
    ...
    for d in data['posts']['data']: #Parse out paging meta data
        time.sleep(.5)
        if('message' not in d.keys() or "New Orleans!" in d['message']):  #TODO: Handle Special case text with unicode chars
            continue
        print("curl -X POST \"https://graph.instagram.com/v18.0/"+page_id+"/feed\" -H \"Content-Type: application/json\" -d \n\'{\"message\":\""+d['message']+"\", \n  \"link\":\"https://instagram.com/pjkrupski/feed\",\n  \"published\":\"true\", \n  \"scheduled_publish_time\":\""+str(publish_date)+"\", \n}\'",flush=True)
    print("\n\n\n",flush=True)
...
# Transfer likes 
with open('fb_likes_data.json', 'r') as file:
    data = json.load(file)
